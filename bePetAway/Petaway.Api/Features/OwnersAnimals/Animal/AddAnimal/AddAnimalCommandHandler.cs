...
namespace Petaway.Api.Features.OwnersAnimals.Animal.AddAnimal
{
    public class AddAnimalCommandHandler : IAddAnimalCommandHandler
    {
        ...
        public async Task HandleAsync(AddAnimalCommand command, string ownerEmail, CancellationToken cancellationToken)
        {
            var owner = await OwnersAnimalsRepository.GetByEmailAsync(ownerEmail, cancellationToken) as OwnersAnimalsDomain;
