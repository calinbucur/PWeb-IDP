...
const Feed = (props) => {
  ...
  for (const key in profData) {
    ...
    objs.push(<div className = {high === key?"Feed-item Feed-highlight":"Feed-item"} key = {key} onClick={()=>{
      ...
      {(idToken && (idToken['https://PetAway.com/role'] === 'owner' || (idToken['https://PetAway.com/role'] === 'foster' && profData[key].status != 'home'))) && <header className = "Feed-pet-status">Status: {profData[key].status}</header>}
      {idToken && idToken['https://PetAway.com/role'] === 'foster' && profData[key].status === 'home' && <button className='Feed-btn'>
